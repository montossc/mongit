...
function isFiniteCoordinate(value: number): boolean {
	return Number.isFinite(value) && Math.abs(value) <= MAX_DRAW_COORDINATE;
}

function isSafeLine(x1: number, y1: number, x2: number, y2: number): boolean {
	return (
...
export function renderGraph(
	...
	const graphTextStartX =
		GRAPH_PADDING_LEFT + laneCount * LANE_WIDTH + TEXT_PADDING_LEFT;
...
function drawRowHighlights(
	...
	const { selectedId, hoveredId, selectedBg, hoverBg, canvasWidth, scrollTop } =
		options;
...
function drawEdges(
	...
	const groups = new Map<
		string,
		{ color: string; isMerge: boolean; segments: GraphSegment[] }
	>();

	for (const segment of segments) {
		// Include edges that cross through the visible range, not just endpoints.
		// Long-span segments still render if any portion intersects [visible.first, visible.last].
		if (!segmentIntersectsVisibleRows(segment, visible)) {
			continue;
		}

		const color =
			theme.colors[segment.color % theme.colors.length] ?? theme.accentColor;
...
function drawNodes(
	...
	for (let row = visible.first; row <= visible.last; row++) {
		...
		const color =
			theme.colors[node.color % theme.colors.length] ?? theme.accentColor;
	...
	if (selectedId) {
		...
		if (selectedNode && isRowVisible(selectedNode.row, visible)) {
			...
			ctx.beginPath();
			ctx.fillStyle =
				theme.colors[selectedNode.color % theme.colors.length] ??
				theme.accentColor;
...
function drawCommitText(
	...
	const hashFont = `500 ${FONT_SIZE}px ${theme.fontMono}`;
	...
	for (let row = visible.first; row <= visible.last; row++) {
		...
		const metaWidth = measure(COMMIT_TEXT_FONT, meta);
		const hash = node.data.id.slice(0, 7);

		const hashText = `${hash} `;
		const hashWidth = measure(hashFont, hashText);

		const availableForMessage = Math.max(
			0,
			rightEdge - textStartX - hashWidth - metaWidth - 20,
		);

		setFont(COMMIT_TEXT_FONT);
